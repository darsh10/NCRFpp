...
def evaluate(data, model, name, nbest=None):
    ...
    for batch_id in tqdm(range(total_batch)):
        ...
        #acc, p, r, f, d_acc, predictions, unequal_lists = get_ner_fmeasure(gold_label, pred_label, data.tagScheme)
        #speed = 0.0
        #if nbest and not data.sentence_classification:
        #    yield speed, d_acc, p, r, f, nbest_pred_result, scores[batch_wordrecover].cpu().data.numpy().tolist(), predictions, unequal_lists
        #else:
        #    yield speed, d_acc, p, r, f, pred_label, scores[batch_wordrecover].cpu().data.numpy().tolist(), predictions, unequal_lists
        pred_results += pred_label
        gold_results += gold_label
    decode_time = time.time() - start_time
    speed = len(instances)/decode_time
    acc, p, r, f, d_acc, predictions, unequal_lists = get_ner_fmeasure(gold_results, pred_results, data.tagScheme)
    if nbest and not data.sentence_classification:
        return speed, d_acc, p, r, f, nbest_pred_results, pred_scores, predictions, unequal_lists
    return speed, d_acc, p, r, f, pred_results, pred_scores, predictions, unequal_lists
...
def train(data):
    ...
    for idx in range(data.HP_iteration):
        ...
        if total_loss > 1e8 or str(total_loss) == "nan":
            ...
            exit(1)
        # continue
        speed, acc, p, r, f, _, _, predictions, unequal = evaluate(data, model, "dev")
        ...
        if acc > best_dev:
            ...
            best_index = idx
            # ## decode test
            speed, acc, p, r, f, _, _, test_predictions, test_unequals = evaluate(data, model, "test")
        ...
        gc.collect()
    print("Best performance dev acc:  %.4f, at index %d" %(best_dev, best_index))
    test_texts = []
    test_golds = []
    f = open("error_analysis.txt","w")
    for i in range(len(data.dev_texts)):
        test_texts.append(data.dev_texts[i][0])
        test_golds.append(data.dev_texts[i][-1])
    assert len(test_unequals) == len(test_predictions)
    assert len(test_unequals) == len(test_texts)
    for unequal, test_text, test_gold, test_prediction in zip(test_unequals, \
            test_texts, test_golds, test_predictions):
        if unequal:
            f.write(" ".join(test_text).strip()+"\n"+\
                    " ".join(test_gold).strip()+"\n"+\
                    " ".join(test_prediction).strip()+"\n")
    f.close()
...
def load_model_decode(data, name):
    ...
    start_time = time.time()
    speed, acc, p, r, f, pred_results, pred_scores, predictions, unequal = evaluate(data, model, name, data.nbest)
    end_time = time.time()
